...
![](Docs_resource/bomb_defusal_manual_cover.png)

**Cẩm nang Tiếng Anh:** [https://www.bombmanual.com/](https://www.bombmanual.com/)\
...
Chi tiết về phần cứng sẽ được giới thiệu sau.
...
Note over gui, ser: Start up system
...
Note over gui, ser: Touch display
user->>gui: Touch screen
activate gui
alt GUI thread
    gui->>gui: Process function
    activate gui
    gui->>lib: Set function common data
    activate lib
    deactivate lib
    deactivate gui
    deactivate gui
...
alt SER thread
    ...
    activate ser
    activate gui
    gui->>lib: Get function common data status
    activate lib
    lib-->>gui: 
    deactivate lib
    alt Function common data status is changed
        gui->>gui: Update function display
        activate gui
        deactivate gui
    end
    deactivate gui
...
actor user as User
box Green GUI
    participant uiev as ui_events
end
box Purple LIB
    participant cdata as CommonData
    participant clib as CommonLibrary
end
participant ser as SER

user->>uiev: Touch screen
activate uiev
alt GUI thread
    uiev->>uiev: Callback function
    activate uiev
    uiev->>uiev: Internal processing
    activate uiev
    uiev->>clib: Use library
    activate clib
    clib-->>uiev: 
    deactivate clib
    uiev->>cdata: Get/Set common data
    activate cdata
    cdata->uiev: 
    deactivate cdata
    deactivate uiev
...
alt SER thread
    ser->>clib: Update screen
    activate ser
    activate clib
    clib->>uiev: Run update function
    activate uiev
    uiev->>cdata: Get common data status
    activate cdata
    cdata-->>uiev: 
    deactivate cdata
    alt Common data status is changed
        uiev->>uiev: Update display
        activate uiev
        deactivate uiev
    end
    deactivate uiev
...
user->>uiev: Touch switch A [SwA]
activate uiev
alt GUI thread
    uiev->>uiev: OnSwitchAChange()
    activate uiev
    uiev->>uiev: Process reading switch state
    activate uiev
    uiev->>clib: Use library if necessary
    activate clib
    clib-->>uiev: 
    deactivate clib
    uiev->>cdata: Set switch state value to common data [SwAValue]
    activate cdata
    cdata->uiev: 
    deactivate cdata
    deactivate uiev
...
alt SER thread
    ...
    activate ser
    activate clib
    clib->>uiev: AutoUpdate()
    activate uiev
    uiev->>cdata: Get common data status [SwAValue]
    activate cdata
    cdata-->>uiev: 
    deactivate cdata
    alt Value of [SwAValue] is changed
        uiev->>uiev: Set value of [SwAValue] to label [LbA]
        activate uiev
        deactivate uiev
    end
    deactivate uiev
